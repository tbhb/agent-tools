...
`release` and `review-release-version` live under `.claude/skills/` with no `.apm/` counterpart, which makes them the first repo-local primitives here. They release this repository, so no consumer should receive them. That placement passes `apm install --frozen`, `apm audit --ci`, and the working-tree drift check in `apm-package.yml` untouched. The installer leaves a file it never deployed alone. The drift check reads `git status`, and a committed file leaves that clean. The skill-scanner job reads `.apm/skills` and never sees it. `skill-limits.sh` and `check-script-hygiene.sh` do, because each derives the local-only set by asking whether `.apm/` holds a skill of the same name, so the next one arrives covered.
