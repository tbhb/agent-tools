...
The skill adds the judgement the tasks can't hold. It confirms the version before dispatching anything, which matters because the automatic path derives one from the commit types and #25 exists because that differed from what the operator wanted. It waits on the dispatched run through its own watcher, since `gh workflow run` returns before the workflow has tagged anything. Guard hooks refuse local tagging, local bumping, and a dispatch that goes around the task, because a tag made here carries no signature while looking exactly like one that does.

`release` lives under `.claude/skills/` with no `.apm/` counterpart, which makes it the first repo-local primitive here. It releases this repository, so no consumer should receive it. That placement passes `apm install --frozen`, `apm audit --ci`, and the working-tree drift check in `apm-package.yml` untouched. The installer leaves a file it never deployed alone. The drift check reads `git status`, and a committed file leaves that clean. The skill-scanner job reads `.apm/skills` and never sees it. `skill-limits.sh` and `check-script-hygiene.sh` do, because each derives the local-only set by asking whether `.apm/` holds a skill of the same name, so the next one arrives covered.
