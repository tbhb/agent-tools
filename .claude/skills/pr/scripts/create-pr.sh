...
#
# The reviewer runs read-only and cannot sign its own verdict, so a
# PostToolUse hook records the digest of whatever it cleared. Comparing
# that against the draft on disk is what makes an edit-after-review read
# as unreviewed.
...
[ "$(cat "$STAMP")" = "$(digest "$DRAFT")" ] ||
  die "${DRAFT} changed after review-pr-description signed off, so the reviewed text
and the text about to be published are no longer the same. Review it again."
printf 'signed\n'
...
if [ -n "$open_number" ]; then
  ...
  gh pr edit "$open_number" --title "$title" --body-file "$body_file"
  # Labels and the milestone are additive on an edit, so a repeat run
  # tops up what the pull request carries rather than replacing it.
  while IFS= read -r item; do
    [ -n "$item" ] || continue
    gh pr edit "$open_number" --add-label "$item" >/dev/null
  done <<EOF
$labels
EOF
  if [ -n "$milestone" ]; then
