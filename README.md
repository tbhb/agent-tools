...
| `check-markdown` | hook | `PreToolUse` gate on `Write` and `Edit` that refuses Markdown whose paragraphs span more than one line. |
...
| Caller | Invocation |
| --- | --- |
| Claude `PreToolUse` | `check-markdown --hook` reads the payload on stdin and denies the `Write` or `Edit` |
| pre-commit | `check-markdown FILES` reports the offending line ranges and exits nonzero |
...
Note that the two legs install separately. prek builds its own copy from `language: golang`. The Claude hook instead resolves `check-markdown` on PATH, which comes from `go install`. A consumer wanting both adds the hook and runs the install.
...
- `check-markdown` refuses Markdown whose paragraphs span more than one line.
